...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/matrixx/build/envsetup.sh | tr '()' '  ' | $one_true_awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/matrixx/build/envsetup.sh | tr '()' '  ' | $one_true_awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function _lunch_meat()
{
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${MATRIXX_BUILD}" ]]; then
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/matrixx/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product $release
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/matrixx/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
. $ANDROID_BUILD_TOP/vendor/matrixx/build/envsetup.sh
